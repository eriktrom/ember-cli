...
module.exports = Command.extend({
  ...
  run: function(commandOptions) {
    ...
    // run a check to ensure host + port are open on all interfaces
    // this would work in theory
    //
    // run this: python -m SimpleHTTPServer 8005
    // then npm link to your project and run ember serve
    function ensurePort() {
      return Promise.resolve(getPort({ host: '0.0.0.0', port: 8005 }))
        .then(function(port) {
          console.log("port is %o", port);
          return port;
        });
    }
    ...
    var liveReloadPort = ensurePort(commandOptions.liveReloadPort);
